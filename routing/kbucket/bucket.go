...
// Split splits a buckets peers into two buckets
// the methods receiver will have peers with CPL equal to cpl,
// the returned bucket will have peers with CPL
...
func (b *Bucket) Split(cpl int, target peer.ID) *Bucket {
	...
	e := b.list.Front()
	for e != nil {
		peerCPL := commonPrefixLen(e.Value.(peer.ID), target)
		if peerCPL > cpl {
			cur := e
